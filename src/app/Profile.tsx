...
export default function Profile(): JSX.Element {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)] flex flex-col p-8">
      ...
      <p className="text-[var(--color-subtext)] mb-8">Manage your profile, orders, and preferences</p>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-80 bg-[var(--color-card)] dark:bg-[var(--color-card)] rounded-2xl shadow-lg border border-[var(--color-border)] p-6 flex flex-col transition-all">
          <div className="flex flex-col items-center mb-6">
            <div className="relative">
              ...
              <span className="absolute bottom-0 right-0 bg-[var(--color-primary)] text-white p-1 rounded-full text-xs cursor-pointer">✎</span>
            </div>
            <h2 className="mt-4 font-semibold text-lg">{profile.name}</h2>
            <p className="text-[var(--color-subtext)] text-sm">{profile.email}</p>
            <span className="mt-2 bg-[var(--color-primary)] text-white text-xs px-3 py-1 rounded-full">Premium Member</span>
          </div>
          ...
          <nav className="flex flex-col gap-2 mt-4">
            {[
              ...
              { label: "My Orders", count: 4 },
              { label: "Wishlist", count: 3 },
              { label: "Cart", count: 2 },
              { label: "Payment Method", count: 0 },
            ].map((item) => (
              <button
                key={item.label}
              ...
              >
                ...
                {item.count > 0 && (
                  <span className="bg-[var(--color-primary)] text-white text-xs px-2 py-0.5 rounded-full">{item.count}</span>
                )}
              </button>
            ))}
          </nav>

          <button className="mt-auto text-red-500 font-semibold text-left">Sign Out</button>
        </aside>
        ...
        <section className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-[var(--color-card)] dark:bg-[var(--color-card)] rounded-2xl shadow-lg border border-[var(--color-border)] p-6">
            <h3 className="font-semibold text-lg mb-1">Profile Overview</h3>
            <p className="text-[var(--color-subtext)] mb-4">Manage your personal and payment information</p>

            {/* Basic Information */}
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                <h4 className="font-semibold text-[var(--color-text)]">Basic Information</h4>
                <button className="text-[var(--color-primary)] text-sm">✎ Edit All</button>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={f.label} className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg p-4">
                    <p className="text-[var(--color-subtext)] text-xs">{f.label}</p>
                    <p className="text-[var(--color-text)] font-medium">{f.value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                <h4 className="font-semibold text-[var(--color-text)]">Account Details</h4>
                <button className="text-[var(--color-primary)] text-sm">✎ Edit All</button>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Membership Tier", value: "Gold Member" },
                ].map((f) => (
                  <div key={f.label} className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg p-4">
                    <p className="text-[var(--color-subtext)] text-xs">{f.label}</p>
                    <p className="text-[var(--color-text)] font-medium">{f.value}</p>
            ...
            {/* Security */}
            <div>
              <h4 className="font-semibold text-[var(--color-text)] mb-2">Security</h4>
      ...
      <button
        onClick={toggleTheme}
        className="absolute top-4 right-4 p-2 rounded-full bg-[var(--color-primary)] text-white"
      >
        Toggle Theme
      </button>
    </div>
